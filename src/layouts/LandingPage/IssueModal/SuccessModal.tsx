...
import { ExportedKeyPair, ImportedKeyPair } from "../../../models/Passkey";
...
const webauthnRegister = async (email: string, name: string) => {
    const makeCredentialOptionsResp = await fetch(
        "https://localhost:44329/makeCredentialOptions",
        {
            method: "POST",
            headers: {
    ...
    const registrationResp = await fetch(
        "https://localhost:44329/makeCredential",
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
...
export const deriveEncryptionKey = (keypair:CryptoKeyPair)=> {
...
export const importKeyPair = async (exportedKeypair:ExportedKeyPair)=>{

    const importedPrivateKey = await  window.crypto.subtle.importKey("jwk", exportedKeypair.privateKey,         {
        name: "ECDH",
        namedCurve: "P-256"
      }, true, ["deriveKey"])
      const importedPublicKey = await  window.crypto.subtle.importKey("jwk", exportedKeypair.publicKey,         {
        name: "ECDH",
        namedCurve: "P-256"
      }, true, [])
      const importedKeyPair:ImportedKeyPair = {
...
const webauthnWriteLargeBlob = async (
    email: string,
    ...
    alert(
        "Almost done!  Your browser will authenticate you one last time to confirm writing an iCloud-backed passkey."
    );

    const p256KeyPair = await window.crypto.subtle.generateKey(
        {
          name: "ECDH",
          namedCurve: "P-256"
        },
    ...
    const exportedKeyPair = await exportKeyPair(p256KeyPair)

    let importedKeyPair = await importKeyPair(exportedKeyPair)
    const encKey = await deriveEncryptionKey(importedKeyPair)


    const largeBlobBytes = new TextEncoder().encode(
        JSON.stringify(exportedKeyPair)
    );
    const assertionOptionsResp = await fetch(
        "https://localhost:44329/assertionOptions",
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
    ...
    const authenticationResp = await fetch(
        "https://localhost:44329/makeAssertion",
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
    ...
    if (authenticationRespJSON && authenticationRespJSON.status === "ok") {
        ...
        console.log(
            ...
            JSON.stringify(authenticationRespJSON, null, 2)
        );
...
const webauthnRegisterAndWriteLargeBlob = async (
    email: string,
    name: string,
    credentialJson: string
) => {
    ...
    const largeBlobBytes = new TextEncoder().encode(
        JSON.stringify(await crypto.subtle.exportKey("jwk", encKey))
    );
    const assertionOptionsResp = await fetch(
        "https://localhost:44329/assertionOptions",
        {
                ...
                {isVisible ? (
                    ...
                    >
                        {credentialSendSuccess ? (
                            ...
                            <div className="absolute bottom-0 left-0 right-0 top-0 z-30 cursor-pointer bg-black bg-opacity-50"></div>
                        )}
                                        ...
                                        <div className="flex w-full flex-col items-start gap-4 p-4 md:p-6">
                                            ...
                                            <button
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    await webauthnWriteLargeBlob(
                                                        userEmail,
                                                        credentialJson
                                                    );
                                                    setCredentialSendSuccess(
                                                        true
                                                    );
                                                }}
                                            >
